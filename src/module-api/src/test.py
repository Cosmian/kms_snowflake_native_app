...
def encrypt_aes(key_id, num_messages):
    data = [_generate_random_string(10) for _ in range(num_messages)]
    encryptions = []

    start_time = time.perf_counter()
    for i in range(0,len(data)):
        enc = create_aes_gcm_encrypt_request(key_id=key_id, data=data[i].encode('utf-8'))
        encryptions.append(enc)
    end_time = time.perf_counter()
    print(
        f"time for creating {num_messages} requests: {round((end_time - start_time) * 1000, 3)} milliseconds"
    )
    results = []
    start_time = time.perf_counter()
    ...
    end_time = time.perf_counter()
    print(
        f"time taken to postprocess {num_messages} messages: {round((end_time - start_time) * 1000, 3)} milliseconds"
    )
    return results

def encrypt_aes_single(key,data):
    start_time = time.perf_counter()
    res = encrypt_with_aes_gcm(key_id=key, cleartext=data.encode('utf-8'))
    end_time = time.perf_counter()
    print(
        f"time taken to encrypt one message: {round((end_time - start_time) * 1000, 3)} milliseconds"
    )
    return res

def decrypt_aes(key_id, data):
    decryptions = []
    start_time = time.perf_counter()
    for i in range(0,len(data)):
        enc = create_aes_gcm_decrypt_request(key_id=key_id, ciphertext=data[i])
    ...
    end_time = time.perf_counter()
    print(
        f"time taken to postprocess {len(data)} messages: {round((end_time - start_time) * 1000, 3)} milliseconds"
    )
